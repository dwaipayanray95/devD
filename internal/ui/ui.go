...
// WrapText wraps a given string to a max width (ignoring ANSI escape code sequences when calculating line lengths).
func WrapText(text string, maxWidth int) string {
	if maxWidth <= 0 {
		return text
	}
	// Use lipgloss style wrapping to properly preserve ANSI escape sequences without breaking ANSI color strings
